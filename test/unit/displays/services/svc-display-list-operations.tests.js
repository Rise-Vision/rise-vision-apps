...
describe('service: DisplayListOperations:', function() {
  ...
  describe('License:', function() {
    it('should exist:', function() {
      expect(displayListOperations.operations[1].name).to.equal('License');
      expect(displayListOperations.operations[1].actionCall).to.be.a('function');
      expect(displayListOperations.operations[1].beforeBatchAction).to.be.a('function');
      expect(displayListOperations.operations[1].groupBy).to.equal('companyId');
      expect(displayListOperations.operations[1].filter).to.deep.equal({
        playerProAuthorized: false
      });
      expect(displayListOperations.operations[1].requireRole).to.equal('da');
    });
    ...
    describe('actionCall:', function() {
      ...
      it('should license displays', function() {
        displayListOperations.operations[1].actionCall(selected);
    ...
    describe('beforeBatchAction:', function() {
      ...
      it('should show error if selected list is empty', function(done) {
        displayListOperations.operations[1].beforeBatchAction([])
          .catch(function() {
      ...
      it('should trigger purchase if license count is not enough', function(done) {
        ...
        displayListOperations.operations[1].beforeBatchAction(selected)
          .catch(function() {
