...
.controller("NewFolderModalCtrl", ["$scope","$modalInstance", "filesFactory","storage",
    "$rootScope", "$translate",
function($scope, $modalInstance, filesFactory, storage, $rootScope, $translate) {
  ...
  $scope.ok = function() {
    ...
    if ($scope.folderName !== "") {
      ...
      storage.createFolder(decodeURIComponent(filesFactory.folderPath || "") + $scope.folderName).then(function (resp) {
        ...
        if (resp.code === 403 && resp.message.indexOf("restricted-role") === -1) {
