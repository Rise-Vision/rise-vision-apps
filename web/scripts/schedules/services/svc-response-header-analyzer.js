angular.module( "risevision.schedules.services" )
  .factory( "responseHeaderAnalyzer", [ "$log", "$http", "$q", 
    ...
    factory.validate = function(url) {
      var message = "The owner of the Web Page at the URL provided does not allow the page to be embedded within an iFrame. If possible, please contact the Web Page owner to discuss ";
      var deferred = $q.defer();

      factory.getOptions(url).then(function(options){
        if (options.includes( "frame-ancestors" )) {
          deferred.reject(message+' (<a target="_blank" href="https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy/frame-ancestors">content-security-policy / frame-ancestors</a>).');
        } else if (options.includes( "X-Frame-Options" )) {
          deferred.reject(message+' (<a target="_blank" href="https://developer.mozilla.org/en-US/docs/Web/HTTP/X-Frame-Options">X-Frame-Options</a>).');
        } else {
